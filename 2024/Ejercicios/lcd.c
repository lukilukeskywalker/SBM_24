...
#define DISPLAY_BYTES_PER_ROW (DISPLAY_WIDTH)
...
void symbolToPosition(custom_symbol_t *symb){
    ...
    uint8_t i, j;
    uint8_t x_pos = symb->x_pos; // X position in the display buffer
    uint8_t y_pos = symb->y_pos; // Y position in the display buffer
    uint8_t x_size = symb->x_size; // Width of the symbol
    uint8_t y_size = symb->y_size; // Height of the symbol (in rows/pages)

    // Iterate over each page (y-axis chunks of 8 pixels)
    for (j = 0; j < y_size; j++) {
        // Iterate over each column (x-axis)
        for (i = 0; i < x_size; i++) {
            // Calculate the position in the custom symbol buffer
            uint8_t buf_index = j * x_size + i;

            // Calculate the position in the local buffer
            uint16_t local_index = (y_pos + j) * 128 + (x_pos + i);

            // Copy the symbol data from the input buffer to the local buffer
            buffer[local_index] = symb->buf[buf_index];
        }
    }
...
uint8_t* getSymbolPointer(uint8_t symbol) {
    // Calculate the offset for the symbol
    uint16_t offset = 25 * (symbol - ' ');

    // Return a pointer to the symbol's data
    return &Arial12x12[offset];
}

void copySymbolToBuffer(uint8_t symbol, custom_symbol_t *dest) {
    uint16_t offset = 25 * (symbol - ' '); // Calculate the starting offset of the symbol
    uint8_t width = Arial12x12[offset];   // Width of the symbol
    uint8_t height = 12;                  // Fixed height (12 pixels)
    
    // Allocate memory for the buffer based on width and height
    uint16_t buf_size = width * height / 8; // Size in bytes
    dest->buf = (uint8_t *)malloc(buf_size); // Allocate memory for the symbol's data
    ...
    // Copy the data from Arial12x12 to the destination buffer
    for (uint8_t i = 0; i < height; i++) {
        // Each row contains data alternated between value1 and value2
        dest->buf[i * 2]     = Arial12x12[offset + i * 2 + 1];
        dest->buf[i * 2 + 1] = Arial12x12[offset + i * 2 + 2];
    }

    // Set the dimensions and position in the struct
    dest->x_size = width;   // Set the width of the symbol
    dest->y_size = height;  // Set the height of the symbol
    dest->x_pos = 0;        // Default X position
    dest->y_pos = 0;        // Default Y position
}
