...
extern GPIO_InitTypeDef GPIO_InitStruct;//variable para los pines
...
char arriba[6];
void Pintar_Arriba(){
	sprintf(arriba,"Arriba");
	for(int i=0; i<6; i++)
		symbolToLocalBuffer_L1(arriba[i]);
}
...
void Pintar_Abajo(){
	sprintf(abajo,"Abajo");
	for(int i=0; i<5; i++)
		symbolToLocalBuffer_L1(abajo[i]);
}
char centro[6];
void Pintar_Centro(){
	sprintf(centro,"Centro");
	for(int i=0; i<6; i++)
		symbolToLocalBuffer_L1(centro[i]);
}
char derecha[7];
void Pintar_Derecha(){
	sprintf(derecha,"Derecha");
	for(int i=0; i<7; i++)
		symbolToLocalBuffer_L1(derecha[i]);
}
char izquierda[9];
void Pintar_Izquierda(){
	sprintf(izquierda,"Izquierda");
	for(int i=0; i<9; i++)
		symbolToLocalBuffer_L1(izquierda[i]);
}
